...
public:
    ...
    //
    // flowRadXYcomp should be motion-compensated (body-rate removed) and expressed in the body frame
    // (or the same frame used by sensorOffsetBody).
    void fuseOFData(const Vector2F &flowRadXYcomp,   // optical flow LOS rates (rad/s)
                    const ftype flowRadAcc,          // 1-sigma accuracy (rad/s)
                    const ftype heightAGL,           // height above ground level (m)
                    const ftype heightVar,           // height variance (m^2)
                    const ftype roll,                // main EKF roll (rad)
                    const ftype rollVar,             // roll variance (rad^2)
                    const ftype pitch,               // main EKF pitch (rad)
                    const ftype pitchVar,            // pitch variance (rad^2)
                    const ftype vD,                  // NED down velocity (m/s)
                    const Vector3F &bodyRates,       // body rates p,q,r (rad/s)
                    const Vector3F &sensorOffsetBody // flow sensor position offset in body frame (m)
                    );
    ...
    // log EKFGSF data on behalf of an EKF caller. id0/id1/id2 are the
    // IDs of the messages to log, e.g. LOG_NKY0_MSG, LOG_NKY1_MSG, LOG_NKY2_MSG
    void Log_Write(uint64_t time_us, LogMessages id0, LogMessages id1, LogMessages id2, uint8_t core_index);
...
private:
    ...
    struct EKF_struct {
        ftype X[5];     // Vel North (m/s),  Vel East (m/s), yaw (rad), Pos North (m), Pos East (m)
        ftype P[5][5];  // covariance matrix
        
        ftype S_vel[2][2];      // N,E velocity innovation covariance (m/s)^2
        ftype innov_vel[2];     // velocity innovation (m/s)

        ftype S_flow[2][2];     // optical flow LOS innovation covariance (rad/s)^2
        ftype innov_flow[2];    // optical flow LOS innovation (rad/s)
        ftype innov_flow_vel[2]; // optical-flow-derived speed innovation proxy (m/s)

        ftype S_pos[2][2];      // N,E position innovation covariance (m)^2
        ftype innov_pos[2];     // position innovation (m)

        // Generic innovation terms used for Gaussian model weighting.
        ftype S[2][2];
        ftype innov[2];
    };
    ...
    // Runs the state and covariance update for the selected EKF using an optical flow LOS-rate measurement.
    // roll/pitch/vD/bodyRates/sensorOffsetBody/rng are treated as external inputs.
    bool correctOF(const uint8_t mdl_idx,
    ...
    // Runs the state and covariance update for the selected EKF using an NE position measurement
    // Returns false if the state and covariance correction failed
    bool correctPos(const uint8_t mdl_idx, const Vector2F &pos, const ftype posObsVar);
